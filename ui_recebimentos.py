# ui_recebimentos.py (Versão com correção definitiva de tipo de dado)
            ...
            if st.session_state.edit_parcela_id == parcela_id:
                ...
                with st.container(border=True):
                    ...
                    if row['status'] == 'Pendente':
                        status_col = cols[3].columns([1,1,1]); status_col[0].warning("PENDENTE")
                        if status_col[1].button("✏️", key=f"edit_{parcela_id}", help="Editar Parcela"):
                            st.session_state.edit_parcela_id = parcela_id; st.rerun()
                        if status_col[2].button("🗑️", key=f"del_{parcela_id}", help="Excluir Parcela"):
                            delete_parcela_plano(parcela_id); st.rerun()
                            ...
                            if contas.empty:
                                ...
                                with st.form(f"form_pay_{row['id']}", clear_on_submit=True):
                                    ...
                                    if st.form_submit_button("Confirmar Pagamento"):
                                        if conta_id is None:
                                            st.error("ERRO: Nenhuma conta de destino foi selecionada.")
                                        else:
                                            # A CORREÇÃO DEFINITIVA ESTÁ AQUI: Convertendo para int()
                                            registrar_pagamento_parcela(row['id'], {
                                                'valor_pago': val_pago, 
